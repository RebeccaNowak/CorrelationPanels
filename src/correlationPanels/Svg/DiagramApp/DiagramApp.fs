...
open Aardvark.UI.Primitives
  ...
  module DiagramApp = 
    ...
    let init : DiagramApp =
      ...
      let _p2 = (V2d (RectangleStack.width s1 * 2.0, 0.0))
      let _p3 = (V2d (RectangleStack.width s1 * 4.0, 0.0))
    ...
    let view (model : MDiagramApp) =
      let foo = 
        Svgplus.RectangleStack.view >> UIMapping.mapAListId  
      let rdnMap = model.rectangleStacks 
